...
func (s *authScheduler) mixedUnavailableErrorLocked(providers []string, model string, predicate func(*scheduledAuth) bool) error {
	...
	return wrapUnavailable(earliest, now, lastCandidateErr)
}
...
// relabeling the failure as missing auth. Otherwise use the cooldown-aware
// unavailable error so Retry-After information survives.
func wrapUnavailable(earliest, now time.Time, lastCandidateErr error) error {
